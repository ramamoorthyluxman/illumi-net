...
class PatchRelightingDataset(Dataset):
    ...
    def calculate_patches(self):
        ...
        # Case 2: Adjust patch size
        if desired_patches < total_patches:
            # Increase patch size
            while patches_x * patches_y > desired_patches:
                if patches_x > patches_y:
                    patches_x -= 1
                else:
                    patches_y -= 1
        else:
            # Decrease patch size
            while (patches_x + 1) * (patches_y + 1) <= desired_patches:
        ...
        # Ensure patch size is not smaller than original
        new_width = max(new_width, patch_width)
        new_height = max(new_height, patch_height)

        # Recalculate patches based on new sizes
        patches_x = max(1, image_width // new_width)
        patches_y = max(1, image_height // new_height)
        total_patches = patches_x * patches_y
