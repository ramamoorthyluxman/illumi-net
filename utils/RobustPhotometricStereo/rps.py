...
class RPS(object):
    ...
    def __init__(self):
        ...
        self.albedo = None  # albedo matrix in numpy array
    ...
    def solve(self, method=L2_SOLVER):
        print("M shape and L shape: ", self.M.shape, " ", self.L.shape)
        if self.M is None:
    ...
    def _solve_l2(self):
        ...
        self.albedo = np.linalg.norm(self.N, axis=1)  # albedo
    ...
    def _solve_l1_multicore(self):
        ...
        if self.foreground_ind is None:
            ...
            self.albedo = np.linalg.norm(self.N, axis=1)
            self.N = normalize(self.N, axis=1)
        else:
            N = np.asarray(normal)
